...
template <typename T, typename L = struct lattice<T>> struct lattice_hash {
    size_t operator()(T const &t) const { return L{}.hash(t); }
};

template <typename T, typename L = struct lattice<T>> struct lattice_equalTo {
    size_t operator()(T const &a, T const &b) const { return L{}.equal(a, b); }
};
