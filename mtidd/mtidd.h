...
namespace mtidd
{
  ...
  private:
    ...
    typedef std::unordered_set<idd_t, //this is use for traversal not modification so it uses normal pointers not shared_ptr
                               idd_hash<V,T,L>,
    ...
    friend idd_t combine1(idd_t lhs, idd_t rhs,
      ...
      if (cached) {
        ...
        if (rhs->is_terminal()) {
          ...
          std::function<idd_t (idd_t)> mapper =
            [&](idd_t x) {
          ...
          if (delta_v < 0) {
            ...
            idd_t_mutable result = std::make_shared<idd<V,T,L>>(mngr, lhs->variable_index, mngr->bottom());
            std::function<idd_t (idd_t, idd_t)> merger =
              [&](idd_t x, idd_t y) {
    ...
    friend void inf_terminal_cover1(idd_t lhs,
      ...
      if (cache.find(lhs) == cache.end()) {
        ...
        if (!lhs->is_terminal()) {
          ...
          for (auto iterator = partition_iterator<const idd<V,T,L>>::covers(lhs->part,var_intv); iterator != end; ++iterator) {
    ...
    friend void partial_overlaps1(idd_t lhs,
                                  std::map<V,interval> & lhs_overlaps,
                                  std::map<V,interval> & rhs_overlaps,
                                  std::map<V,interval> const & box,
                                  idd_set & cache) {
      if (!(cache.find(lhs) == cache.end())) return;
      cache.insert(lhs);
      if (lhs->is_terminal()) return;
 ...
 public:
    ...
    idd(idd_manager<V,T,L>* mngr, int var_idx, idd_t out): variable_index(var_idx), terminal_index(-1), part(new_partition(out)), manager(mngr) {
      computeHash();
    }

    idd(idd_manager<V,T,L>* mngr, int var_idx, const interval & i, idd_t in, idd_t out): variable_index(var_idx), terminal_index(-1), part(new_partition(out)), manager(mngr) {
    ...
    bool equals_structural(idd_t rhs) const {
      return equals_structural(*rhs);
    }
    ...
    friend void partial_overlaps(idd_t lhs,
                                 std::map<V,interval> & lhs_overlaps,
                                 std::map<V,interval> & rhs_overlaps,
                                 std::map<V,interval> const & box) {
      lhs_overlaps.clear();
  ...
  struct idd_hash {
    size_t operator()(std::shared_ptr<idd<V,T,L> const> s) const
    {
      return s->hash();
    }
  };
  ...
  public:
    ...
    V variable_at(int index) {
      return variable_ordering.at(index);
    }

    T terminal_at(int index) {
  ...
  struct idd_pair_hash {
    size_t operator()(std::tuple<std::shared_ptr<idd<V,T,L> const>, std::shared_ptr<idd<V,T,L> const>> const & s) const
    {
