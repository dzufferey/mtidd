...
template <class A> class partition_iterator {
  private:
    typedef typename partition<A>::const_iterator boundaries_iterator;
    typedef std::function<bool(half_interval const &lower_bound, half_interval const &upper_bound, const A &)>
        filter_fun;
    ...
    filter_fun trivial() const {
        return [](half_interval const &, half_interval const &, const A &) { return true; };
    }

    partition_iterator(partition<A> const &boundaries, boundaries_iterator const &position)
        : filter(trivial()), lb(lower_sentinel), itr(position), last(boundaries.cend()) {}

  public:
    using iterator_category = std::forward_iterator_tag;
    using value_type = std::shared_ptr<const A>;
    using difference_type = std::ptrdiff_t;
    using pointer = std::shared_ptr<const A> *;
    using reference = std::shared_ptr<const A> &;

    partition_iterator(partition<A> const &boundaries)
        : filter(trivial()), lb(lower_sentinel), itr(boundaries.cbegin()), last(boundaries.cend()) {
        until_filter();
    }

    partition_iterator(partition<A> const &boundaries, filter_fun filter_elements)
        : filter(filter_elements), lb(lower_sentinel), itr(boundaries.cbegin()), last(boundaries.cend()) {
        until_filter();
    }
    ...
    bool operator==(partition_iterator<A> const &other) const { return itr == other.itr; }

    bool operator!=(partition_iterator<A> const &other) const { return !(*this == other); }

    std::shared_ptr<const A> operator*() const { return std::get<1>(*itr); }

    half_interval const &lower_bound() const { return lb; }
