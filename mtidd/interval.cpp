...
interval_boundary complement(interval_boundary b) { return b == Open ? Closed : Open; }
...
bool is_empty(const interval &i) {
    return get<0>(i) > get<2>(i) || (get<0>(i) == get<2>(i) && (get<1>(i) == Open || get<3>(i) == Open));
}

bool is_singleton(const interval &i) { return get<0>(i) == get<2>(i) && get<1>(i) == Closed && get<3>(i) == Closed; }
...
bool covers(const interval &covering, const interval &covered) {
    return !is_empty(covering) && !is_empty(covered) && starts_after(covering) <= starts_after(covered) &&
...
half_interval ends(const interval &i) { return make_tuple(get<2>(i), get<3>(i)); }
